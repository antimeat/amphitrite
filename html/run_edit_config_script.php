<?php
    ...
    $sites = $_POST['sitesHidden'];
    $header_comments = $_POST["commentsHidden"];            

    // Escape the command arguments to handle special characters correctly
    $escaped_sites_data = escapeshellarg($sites);
    $escaped_header_comments = escapeshellarg($header_comments);
    $escaped_file_name = escapeshellarg($CONFIG_FILE_NAME);

    ob_start();
    passthru("cd $dir && $dir" . "save_config.py $escaped_sites_data $escaped_header_comments $escaped_file_name");
    ...
    <body>                    
        ...
        $(document).ready(function() {
            ...
            $('#resultModal').on('hidden.bs.modal', function () {
                window.location.href = 'edit_config_script.php'; // Redirect URL
            });
        });
        </script>
